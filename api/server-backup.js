...
function formatProject(project) {
  return {
    ...
    status: ValidationStatus[project.status],
    statusCode: project.status,
    submissionTimestamp: project.submissionTimestamp.toString(),
    validationTimestamp: project.validationTimestamp.toString(),
    tokenizationTimestamp: project.tokenizationTimestamp.toString(),
    metadata: JSON.parse(project.metadata || '{}'),
    isReadyForBitbond: project.isReadyForBitbond,
    ipfsUrl: `https://ipfs.io/ipfs/${project.ipfsCID}`,
    submissionDate: new Date(parseInt(project.submissionTimestamp) * 1000).toISOString(),
    validationDate: project.validationTimestamp > 0 ? new Date(parseInt(project.validationTimestamp) * 1000).toISOString() : null,
    tokenizationDate: project.tokenizationTimestamp > 0 ? new Date(parseInt(project.tokenizationTimestamp) * 1000).toISOString() : null
  };
}
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        status: ValidationStatus[proofResult.status],
